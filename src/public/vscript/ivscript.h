...
public:
	...
	bool ValueExists( const char *pszKey )																							{ return GetVM() ? GetVM()->ValueExists( m_hScope, pszKey ) : false; }
	bool SetValue( const char *pszKey, const ScriptVariant_t &value )																{ return GetVM() ? GetVM()->SetValue(m_hScope, pszKey, value ) : false; }
	bool GetValue( const char *pszKey, ScriptVariant_t *pValue )																	{ return GetVM() ? GetVM()->GetValue(m_hScope, pszKey, pValue ) : false; }
	void ReleaseValue( ScriptVariant_t &value )																						{ if (GetVM()) GetVM()->ReleaseValue( value ); }
	bool ClearValue( const char *pszKey)																							{ return GetVM() ? GetVM()->ClearValue( m_hScope, pszKey ) : false; }
