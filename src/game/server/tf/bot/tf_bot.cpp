...
static int Rim_AddTeamBots( int iTeam, int nBotsToAdd )
{
	...
	static const char *s_pszClasses[] = { "scout", "heavyweapons", "pyro" };

	CTFBot::DifficultyType skill = clamp( (CTFBot::DifficultyType)tf_bot_difficulty.GetInt(), CTFBot::EASY, CTFBot::EXPERT );

	int iNumAdded = 0;
	for ( int i = 0; i < nBotsToAdd; ++i )
	{
		const int iClassIndex = GetClassIndexFromString( s_pszClasses[i % ARRAYSIZE( s_pszClasses )] );
		...
		pBot->SetDifficulty( skill );
		pBot->HandleCommand_JoinClass( s_pszClasses[i % ARRAYSIZE( s_pszClasses )] );
		if ( pBot->GetTeamNumber() != iTeam )
		{
			pBot->ChangeTeam( iTeam, false, false );
		}
...
//-----------------------------------------------------------------------------------------------------
static int Rim_AddBotsInternal( void )
{
	int nRedBots = 0;
	int nBlueBots = 0;
	int nRedHumans = 0;
	int nBlueHumans = 0;
	Rim_CountTeams( nRedBots, nBlueBots, nRedHumans, nBlueHumans );

	const int nBaseBotsPerTeam = 4;
	const int nNeedBlue = MIN( 8, MAX( 0, nBaseBotsPerTeam - nBlueBots ) + nRedHumans );
	const int nNeedRed = MIN( 8, MAX( 0, nBaseBotsPerTeam - nRedBots ) + nBlueHumans );

	const int iNumAdded = Rim_AddTeamBots( TF_TEAM_BLUE, nNeedBlue ) + Rim_AddTeamBots( TF_TEAM_RED, nNeedRed );
	if ( iNumAdded > 0 )
	{
		Msg( "RIM: added %d bots (BLU need %d incl. +%d for RED humans, RED need %d incl. +%d for BLU humans).\n",
			iNumAdded, nNeedBlue, nRedHumans, nNeedRed, nBlueHumans );
	}
...
//-----------------------------------------------------------------------------------------------------
void Rim_SpawnDefaultBots( void )
{
	Rim_AddBotsInternal();
}

//-----------------------------------------------------------------------------------------------------
CON_COMMAND_F( rim_add_bots, "Add Scout/Heavy/Pyro bots to both teams (RIM mode).", FCVAR_GAMEDLL )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
	{
		return;
	...
	if ( !TFGameRules() || !TFGameRules()->IsRainbowIsMagicMode() )
	{
		Msg( "rim_add_bots: tf_rim_mode must be 1\n" );
		return;
	}

	const int iNumAdded = Rim_AddBotsInternal();

	Msg( "rim_add_bots: spawned %d bots\n", iNumAdded );
	UTIL_ClientPrintAll( HUD_PRINTTALK, CFmtStr( "RIM: Added %d Scout/Heavy/Pyro bots.", iNumAdded ) );
}
//-----------------------------------------------------------------------------------------------------
static bool OW_IsSafeToSpawnBots( void )
{
	...
	// KOTH/CP setup: bots with seek-and-destroy path into walls and die.
	if ( TFGameRules()->InSetup() )
	{
		return false;
	}
...
static float s_flOWAllowBotSpawnTime = -1.0f;
static int s_nOWPendingBotsRed = 0;
static int s_nOWPendingBotsBlue = 0;
static float s_flOWNextStaggeredSpawnTime = -1.0f;

//-----------------------------------------------------------------------------------------------------
void OW_ResetBotSpawnSchedule( float flDelaySeconds )
{
	s_flOWAllowBotSpawnTime = gpGlobals->curtime + flDelaySeconds;
	s_nOWPendingBotsRed = 0;
	s_nOWPendingBotsBlue = 0;
	s_flOWNextStaggeredSpawnTime = -1.0f;
}

//-----------------------------------------------------------------------------------------------------
static int OW_AddTeamBots( int iTeam, int nBotsToAdd )
{
	if ( nBotsToAdd <= 0 || !OW_IsSafeToSpawnBots() )
	{
		return 0;
	}

	static const char *s_pszClasses[] = { "scout", "heavyweapons", "pyro", "sniper", "medic", "engineer" };
	...
	for ( int i = 0; i < nBotsToAdd; ++i )
	{
		const char *pszClass = s_pszClasses[i % ARRAYSIZE( s_pszClasses )];
		const int iClassIndex = GetClassIndexFromString( pszClass );
		...
		pBot->ChangeTeam( iTeam, false, false );
		pBot->SetDifficulty( skill );
		pBot->HandleCommand_JoinClass( pszClass );
		if ( pBot->GetTeamNumber() != iTeam )
		{
			pBot->ChangeTeam( iTeam, false, false );
		}
...
//-----------------------------------------------------------------------------------------------------
static int OW_GetTargetBotsForTeam( int iTeam, int nRedHumans, int nBlueHumans )
{
	extern ConVar tf_ow_bots_per_team;
	extern ConVar tf_ow_enemy_bonus_on_join;

	const int nBase = clamp( tf_ow_bots_per_team.GetInt(), 1, 11 );
	const int nBonus = clamp( tf_ow_enemy_bonus_on_join.GetInt(), 0, 6 );

	if ( iTeam == TF_TEAM_RED )
	{
		return nBase + ( nBlueHumans > 0 ? nBonus : 0 );
	}
	if ( iTeam == TF_TEAM_BLUE )
	{
		return nBase + ( nRedHumans > 0 ? nBonus : 0 );
	}
...
void OW_QueueNeededBots( void )
{
	...
	const int nTargetRed = OW_GetTargetBotsForTeam( TF_TEAM_RED, nRedHumans, nBlueHumans );
	const int nTargetBlue = OW_GetTargetBotsForTeam( TF_TEAM_BLUE, nRedHumans, nBlueHumans );
	s_nOWPendingBotsRed = MIN( 11, MAX( 0, nTargetRed - nRedBots ) );
	s_nOWPendingBotsBlue = MIN( 11, MAX( 0, nTargetBlue - nBlueBots ) );

	if ( s_nOWPendingBotsRed > 0 || s_nOWPendingBotsBlue > 0 )
	{
		if ( s_flOWNextStaggeredSpawnTime < 0.0f || s_flOWNextStaggeredSpawnTime > gpGlobals->curtime )
		{
			s_flOWNextStaggeredSpawnTime = gpGlobals->curtime;
		}
		Msg( "OW: queued %d RED + %d BLU bots (targets %d / %d).\n",
			s_nOWPendingBotsRed, s_nOWPendingBotsBlue, nTargetRed, nTargetBlue );
	}
}

//-----------------------------------------------------------------------------------------------------
void OW_TickBotSpawnQueue( void )
{
	if ( !OW_IsSafeToSpawnBots() )
	{
		return;
	}

	if ( gpGlobals->curtime < s_flOWAllowBotSpawnTime )
	{
		return;
	}
	...
	if ( gpGlobals->curtime < s_flOWNextStaggeredSpawnTime )
	{
		return;
	}

	int iNumAdded = 0;
	if ( s_nOWPendingBotsBlue > 0 )
	{
		iNumAdded += OW_AddTeamBots( TF_TEAM_BLUE, 1 );
		--s_nOWPendingBotsBlue;
	}
	else if ( s_nOWPendingBotsRed > 0 )
	{
		iNumAdded += OW_AddTeamBots( TF_TEAM_RED, 1 );
		--s_nOWPendingBotsRed;
	}
...
//-----------------------------------------------------------------------------------------------------
void OW_MaintainBotCounts( void )
{
	if ( !TFGameRules() || !TFGameRules()->IsOverwatchMode() )
	{
		return;
...
//-----------------------------------------------------------------------------------------------------
void OW_SpawnDefaultBots( void )
{
	OW_ResetBotSpawnSchedule( 1.0f );
	OW_QueueNeededBots();
}

//-----------------------------------------------------------------------------------------------------
CON_COMMAND_F( ow_add_bots, "Add OW hero bots to both teams.", FCVAR_GAMEDLL )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
	{
		return;
	...
	if ( !TFGameRules() || !TFGameRules()->IsOverwatchMode() )
	{
		Msg( "ow_add_bots: tf_ow_mode must be 1\n" );
		return;
	}
