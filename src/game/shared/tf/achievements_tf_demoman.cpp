...
class CAchievementTFDemoman_DecapitateCloakedSpy : public CBaseTFAchievement
{
	...
	virtual void Event_EntityKilled( CBaseEntity *pVictim, CBaseEntity *pAttacker, CBaseEntity *pInflictor, IGameEvent *event ) 
	{
		...
		if ( pTFVictim && pTFVictim->IsPlayerClass( TF_CLASS_SPY ) && pTFVictim->m_Shared.InCond( TF_COND_STEALTHED ) )
		{
			...
			if ( customdmg == TF_DMG_CUSTOM_DECAPITATION || customdmg == TF_DMG_CUSTOM_HONORBOUND_DUEL )
			{
				IncrementCount();
			}
		}
...
class CAchievementTFDemoman_CritSwordKill : public CBaseTFAchievement
{
	...
	virtual void Event_EntityKilled( CBaseEntity *pVictim, CBaseEntity *pAttacker, CBaseEntity *pInflictor, IGameEvent *event ) 
	{
		...
		if ( (nDamageBits & DMG_CRITICAL) && ( ( event->GetInt( "customkill" ) == TF_DMG_CUSTOM_DECAPITATION ) || event->GetInt("customkill") == TF_DMG_CUSTOM_HONORBOUND_DUEL ) ) )
		{
			IncrementCount();
		}
	}
