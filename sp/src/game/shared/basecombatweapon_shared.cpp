...
//-----------------------------------------------------------------------------
// Purpose: iron sights related functions
//-----------------------------------------------------------------------------
...
void CBaseCombatWeapon::EnableIronsights(void)
{
	...
	if (pOwner->SetFOV(this, pOwner->GetDefaultFOV() + GetIronsightFOVOffset(), IRONSIGHT_IN_TIME))
	{
...
void CBaseCombatWeapon::DisableIronsights(void)
{
	...
	if (pOwner->SetFOV(this, 0, IRONSIGHT_OUT_TIME))
	{
		m_bIsIronsighted = false;
		SetIronsightTime();
	}
...
void CBaseCombatWeapon::Drop( const Vector &vecVelocity )
{
	...
	StopAnimation();
	StopFollowingEntity( );
	SetMoveType( MOVETYPE_FLYGRAVITY); // flygrav
	...
	if ( pObj != NULL )
	{
		AngularImpulse	angImp( RandomFloat(200, 500), RandomFloat(200, 1500), RandomInt(-1, 1) * 2000);
		Vector vecVelocityProxy;
		VectorCopy(vecVelocity, vecVelocityProxy);
		vecVelocityProxy.z += 150;
		pObj->AddVelocity( &vecVelocityProxy, &angImp);
	}
	else
	{
...
void CBaseCombatWeapon::ProcessLadderMove( void )
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
	if (!pOwner)
		return;
	...
	// Holstering
	if (!m_bWeaponIsLowered && (pOwner->GetMoveType() == MOVETYPE_LADDER))
	{
	...
	// Unholstering
	else if (m_bWeaponIsLowered && !(pOwner->GetMoveType() == MOVETYPE_LADDER))
	{
		m_bWeaponIsLowered = false;
		SendWeaponAnim(ACT_VM_DRAW);
		pOwner->GetViewModel()->SetEffects(pOwner->GetViewModel()->GetEffects() & ~EF_NODRAW);
		m_flNextPrimaryAttack = gpGlobals->curtime + GetViewModelSequenceDuration();
		m_flNextSecondaryAttack = m_flNextPrimaryAttack;
	}
