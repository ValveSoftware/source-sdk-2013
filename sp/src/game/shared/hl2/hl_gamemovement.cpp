...
bool CHL2GameMovement::CheckLadderAutoMountCone(CFuncLadder *ladder, const Vector& bestOrigin, float maxAngleDelta, float maxDistToLadder)
{
	...
	if (ladder != NULL &&
	    ((mv->m_flForwardMove > 0.0f) || (player->GetGroundEntity() == NULL)))
	{
		Vector top, bottom;
		ladder->GetTopPosition(top);
		ladder->GetBottomPosition(bottom);
		...
		if (((facingDot > 0.0f && !strafing) || facingalongaxis) &&
			facingladderaxis && closetoladder &&	// make sure player is facing the ladder and is close enough to it
			!reallyclosetoladder)					// BUT make sure he is not too close so he can jump off at any time
		{
			StartForcedMove(true, player->MaxSpeed(), bestOrigin, ladder);
			return true;
		}
...
bool CHL2GameMovement::LadderMove(void)
{
	...
	{
		// If flying through air, allow mounting ladders if we are facing the ladder and we are close
		if (!ladder && !sv_ladder_useonly.GetBool())
		{
			...
			{
				if (CheckLadderAutoMountCone(bestLadder, bestOrigin, 32.0f, 64.0f))
				{
					return true;
				}
			}
